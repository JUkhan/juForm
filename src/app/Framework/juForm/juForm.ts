...
@Component({
    selector: 'juForm',
    directives: [], encapsulation: ViewEncapsulation.None,
    template: '<div></div>',
    styles: [require('./juForm.css')],
...
export class juForm implements OnInit, OnDestroy, OnChanges {
    ...
    showModal(isDisplayed: boolean = true) {
        jQuery(this._elementRef.nativeElement).find('.modal').modal(isDisplayed ? 'show' : 'hide');
        if (isDisplayed) {
...
function getComponent(obj: any) {
    //console.log(obj.tpl);
    @Component({
        selector: 'div', template: obj.tpl, directives: [juSelect, Datetimepicker, CkEditor, juForm, FileSelect]
    })
